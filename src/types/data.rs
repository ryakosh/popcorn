...
impl<'s> SignupData<'s> {
    fn new(uname: &'s str, pwd: &'s str, email: &'s str) -> Result<Self, Errors> {
        ...
        if is_uname && is_pwd && is_email {
            ...
            if let false = is_uname {
                errors.push(Error::UnameInvalid)
            }
            if let false = is_pwd {
                errors.push(Error::PwdInvalid)
            }
            if let false = is_email {
