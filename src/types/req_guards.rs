...
impl<'a, 'r> request::FromRequest<'a, 'r> for ClaimedUser {
    ...
    fn from_request(request: &'a request::Request<'r>) -> request::Outcome<Self, Self::Error> {
        ...
        if let Some(authorization) = authorization {
            let authorization: Vec<_> = authorization.trim().splitn(3, ' ').take(2).collect();
            ...
            if authorization.len() == 2 {
                if authorization[0] == "Bearer" {
                    ...
                    match result {
                        Ok(token) => rocket::Outcome::Success(ClaimedUser(token.claims.sub)),
            ...
            rocket::Outcome::Failure((http::Status::Unauthorized, error::Error::AuthRequired))
        }
    }
}
