...
use crate::types::data::{SigninData, SignupData, RateData};
use crate::types::query::MoviesQuery;
use crate::types::Claims;
use crate::types::req_guards::ClaimedUser;
use models::{Artist, Director, Movie, MovieCompact, User, Writer, NewRating};
use schema::{movies, artists, directors, movies_artists, movies_directors, movies_writers, writers, ratings};
use std::env::var;
...
pub fn rate(claimed_user: &ClaimedUser, movie_id: i32, rate_data: &RateData) ->
    Result<(), Error> {
    
    let conn = connect(&var("DATABASE_URL").expect("Can't find DATABASE_URL environment variable"));
    let rate_data = rate_data.validate()?;
    
    conn.build_transaction().run::<_, diesel::result::Error, _>(|| {
        ...
        let count: i64 = ratings::table.select(diesel::dsl::count(ratings::user_id))
            .first(&conn)
            .expect("Error executing query");
        
        diesel::update(movies::table.find(movie_id))
            .set(movies::score.eq(((movies::score * (count - 1) as f32) + rate_data.rating() as f32) / count as f32))
            .execute(&conn)
            .expect("Error executing query");
