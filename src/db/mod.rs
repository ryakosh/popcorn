...
pub fn movies(movies_query: &MoviesQuery, conn: &Conn) -> Result<Vec<MovieCompact>, Error> {
...
> {
    ...
    let mru = user_id.map(|user_id| {
        let rating = movies_rate::get_users_movie_rating(id, user_id, conn);
        let is_watchlisted = movies_watchlist::is_movie_watchlisted(user_id, id, conn);
        let is_favorite = movies_favorites::is_movie_favorite(user_id, id, conn);
